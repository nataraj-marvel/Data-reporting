-- ============================================================================
-- NAUTILUS REPORTING SYSTEM - CLEAN INSTALLATION SCRIPT (v2)
-- Safe to run on new or partially existing databases
-- Includes AI prompt foreign-key remediation + JSON compatibility fixes
...
BEGIN
    ...
    IF fk_exists > 0 THEN
        SET @ddl = CONCAT('ALTER TABLE ', schema_name, '.', table_name, ' DROP FOREIGN KEY ', fk_name);
...
CALL drop_fk_if_exists('nautilus_reporting2', 'issues', 'fk_issues_prompt');
CALL drop_fk_if_exists('nautilus_reporting2', 'problems_solved', 'fk_solutions_prompt');
CALL drop_fk_if_exists('nautilus_reporting2', 'prompt_files', 'prompt_files_ibfk_1');
CALL drop_fk_if_exists('nautilus_reporting2', 'tasks', 'tasks_ibfk_5');
CALL drop_fk_if_exists('nautilus_reporting3', 'prompt_files', 'prompt_files_ibfk_1');
CALL drop_fk_if_exists('nautilus_reporting3', 'tasks', 'tasks_ibfk_5');
...
CREATE TABLE ai_prompts (
    ...
    CONSTRAINT chk_ai_prompts_context_json CHECK (context_data IS NULL OR JSON_VALID(context_data)),
    FOREIGN KEY (user_id) REFERENCES users(user_id) ON DELETE CASCADE,
    FOREIGN KEY (report_id) REFERENCES daily_reports(report_id) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

DROP PROCEDURE IF EXISTS add_ai_prompt_fk;
DELIMITER //
CREATE PROCEDURE add_ai_prompt_fk(
    IN schema_name VARCHAR(64),
    IN table_name VARCHAR(64),
    IN fk_name VARCHAR(64),
    IN column_name VARCHAR(64),
    IN delete_rule VARCHAR(20)
)
BEGIN
    ...
    IF table_exists > 0 THEN
        ...
        IF fk_exists = 0 THEN
            SET @ddl = CONCAT(
                ...
                ' FOREIGN KEY (', column_name, ') REFERENCES nautilus_reporting.ai_prompts(prompt_id) ON DELETE ', delete_rule
            );
            PREPARE stmt FROM @ddl;
            EXECUTE stmt;
            DEALLOCATE PREPARE stmt;
...
CALL add_ai_prompt_fk('nautilus_reporting2', 'issues', 'fk_issues_prompt', 'prompt_id', 'SET NULL');
CALL add_ai_prompt_fk('nautilus_reporting2', 'problems_solved', 'fk_solutions_prompt', 'related_prompt_id', 'SET NULL');
CALL add_ai_prompt_fk('nautilus_reporting2', 'prompt_files', 'prompt_files_ibfk_1', 'prompt_id', 'CASCADE');
CALL add_ai_prompt_fk('nautilus_reporting2', 'tasks', 'tasks_ibfk_5', 'prompt_id', 'SET NULL');
CALL add_ai_prompt_fk('nautilus_reporting3', 'prompt_files', 'prompt_files_ibfk_1', 'prompt_id', 'CASCADE');
CALL add_ai_prompt_fk('nautilus_reporting3', 'tasks', 'tasks_ibfk_5', 'prompt_id', 'SET NULL');
